exports = module.exports = function (data, noDataOutput) {
    ...
    sanssrRuntime.renderer1 = function (data = {}, noDataOutput, sanssrRuntime, ownerCtx, parentCtx, tagName, sourceSlots) {
        ...
        var $sourceSlots = [];
        html += sanssrRuntime.renderer2({"link": ctx.data.link, "framework": ctx.data.framework, "linkText": ctx.data.linkText}, noDataOutput, sanssrRuntime, ctx, currentCtx, "x-wrap", $sourceSlots);
        $sourceSlots = null;
        return html;
    }
    sanssrRuntime.prototype2 = {
        filters: {
        },
        computed: {
        },
    ...
    sanssrRuntime.renderer2 = function (data = {}, noDataOutput, sanssrRuntime, ownerCtx, parentCtx, tagName, sourceSlots) {
        var _ = sanssrRuntime._;
        var SanData = sanssrRuntime.SanData;
        var html = "";
        var ctx = {
            instance: _.createFromPrototype(sanssrRuntime.prototype2),
            sourceSlots: sourceSlots,
            data: data,
            owner: ownerCtx,
            computedNames: [],
        ...
        var $sourceSlots = [];
        html += sanssrRuntime.renderer3({"link": ctx.data.link, "name": ctx.data.framework, "linkText": ctx.data.linkText}, noDataOutput, sanssrRuntime, ctx, currentCtx, "x-child", $sourceSlots);
        $sourceSlots = null;
        return html;
    }
    sanssrRuntime.prototype3 = {
        filters: {
        },
        computed: {
        },
    ...
    sanssrRuntime.renderer3 = function (data = {}, noDataOutput, sanssrRuntime, ownerCtx, parentCtx, tagName, sourceSlots) {
        var _ = sanssrRuntime._;
        var SanData = sanssrRuntime.SanData;
        var html = "";
        var ctx = {
            instance: _.createFromPrototype(sanssrRuntime.prototype3),
            sourceSlots: sourceSlots,
            data: data,
            owner: ownerCtx,
            computedNames: [],
        ...
        html += "<h3>";
        if (!noDataOutput) {
            html += "<!--s-data:" + JSON.stringify(ctx.data) + "-->";
        }
        html += "see <a";
        html += _.attrFilter("href", ctx.data.link, true);
        html += ">";
        html += (_.escapeHTML(ctx.data.linkText || ctx.data.name));
        html += "</a> to start <b>";
        html += (_.escapeHTML(ctx.data.name));
        html += "</b> framework</h3>";
        return html;
    }
